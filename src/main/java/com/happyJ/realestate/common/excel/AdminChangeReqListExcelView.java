...
/**
 * 변경요청내역 목록을 엑셀파일로 돌려준다
 * @author ydak
 *
 */
@Component
public class AdminChangeReqListExcelView extends ExcelView{
	...
	protected void createData(Map<String,Object> model) {
		...
		int rowNo = 0;
		// 칼럼 타이틀
		String[] colTitles = {"번호", "아이디", "IP", "변경요청일", "승인완료일", "변경메뉴", "상세내용", "미승인 사유", "상태"};
		
		// 제목
		row = worksheet.createRow(rowNo); // row 생성
		cell = row.createCell(0);
		cell.setCellValue("변경요청 내역 리스트");
		...
		if (changeReqList.isEmpty()){
			...
			for (ChangeReqDto changeReq : changeReqList){
				...
				cell.setCellValue(changeReq.getReqUserid());
				cell.setCellStyle(style);
				
				cell = row.createCell(2);
				cell.setCellValue(changeReq.getReqIp());
				cell.setCellStyle(style);
