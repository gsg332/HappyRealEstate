 ...
 *  @fileName : RejectListExcelView.java
 *  @author : yongpal
 *  @since 2016. 5. 19.
 *  @version 1.0 
 *  @see  :  
 ...
 *    DATE	           NAME			DESC
 *     -----------	 ----------   ---------------------------------------
 *     2016. 5. 19.        yongpal       create RejectListExcelView.java
 *  </pre>
 ******************************************************************************/
public class ActionHistoryListExcelView extends ExcelView{
	...
	protected void createData(Map<String,Object> model) {
		...
		List<ActionHistoryDto> actionHistoryList = (List<ActionHistoryDto>) model.get("excelList");
		...
		if (actionHistoryList.isEmpty()){
			...
			for (ActionHistoryDto actionHistory : actionHistoryList){
				
				style = columnStyle("center", "");
				row = worksheet.createRow(rowNo); // row 생성
				
				cell = row.createCell(0);
				cell.setCellValue(actionHistory.getIdx());
				cell.setCellStyle(style);
