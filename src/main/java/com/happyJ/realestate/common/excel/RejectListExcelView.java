 ...
 *  </pre>
 ******************************************************************************/
public class RejectListExcelView extends ExcelView{
	...
	protected void createData(Map<String,Object> model) {
		...
		int rowNo = 0;
		// 칼럼 타이틀
		String[] colTitles = {"신청번호", "아이디", "이름", "신청일시", "반려일시", "반려사유"};
		
		// 제목
		row = worksheet.createRow(rowNo); // row 생성
		cell = row.createCell(0);
		cell.setCellValue("영상 신청 반려 리스트");
		...
		if (applyList.isEmpty()){
			...
			for (ApplyCustomDto apply : applyList){
				...
				cell.setCellValue(apply.getRejectDate());
				cell.setCellStyle(style);
				
				cell = row.createCell(5);
				cell.setCellValue(apply.getVeiRejectReason());
				cell.setCellStyle(style);
