...
<script type="text/javascript">
	$(document).ready(function(){
		...
		pagination("${actionHistoryList[0].totalPage}", "${actionHistoryList[0].currentPage}");
		...
		$("#selRowSize").change(function(){
			...
			<dl class="dl-horizontal" style="position:relative; height:40px">
				<dt style="padding-top:10px">${actionHistoryList[0].listCnt}건 조회&nbsp;&nbsp;|&nbsp;&nbsp;${actionHistoryList[0].currentPage}/${actionHistoryList[0].totalPage} 페이지 </dt>
				<dd style="position:absolute; right:0px; top:10px">
					<select name="selRowSize" id="selRowSize" class="form-control">
						<c:forEach var="row" begin="10" end="30" step="10" varStatus="status">
							<c:choose>
								<c:when test="${row eq actionHistoryList[0].rowSize}">
			...
			<table class="table table-hover table_b text-center">
				...
				<tbody>
				<c:forEach var="list" items="${actionHistoryList}" varStatus="status">
					<tr>
						<%-- <td>${(actionHistoryList[0].listCnt - status.index) - ( (actionHistoryList[0].currentPage - 1)  *  actionHistoryList[0].rowSize ) }</td> --%>
						<td>${list.idx}</td>
						<td>${list.userId}</td>
						<td>${list.accessIp}</td>
						<td>${list.menu}</td>
